...
### Reference: [PortSwigger: User ID Controlled by Request Parameter, with Unpredictable User IDs](https://portswigger.net/web-security/access-control/lab-user-id-controlled-by-request-parameter-with-unpredictable-user-ids)

**Solution:**
1. Log in using the supplied credentials and go to your account page.
2. Observe that the `id` parameter contains an unpredictable user ID.
3. Use Burp Suite to intercept the request and modify the `id` parameter to target another user.
4. Retrieve sensitive information for the targeted user.

## User ID controlled by request parameter with data leakage in redirect
### Reference: [PortSwigger: User ID Controlled by Request Parameter with Data Leakage in Redirect](https://portswigger.net/web-security/access-control/lab-user-id-controlled-by-request-parameter-with-data-leakage-in-redirect)

**Solution:**
1. Log in and observe that a redirect URL contains the user ID parameter.
2. Modify the user ID in the request to test for data leakage.

## User ID controlled by request parameter with password disclosure
### Reference: [PortSwigger: User ID Controlled by Request Parameter with Password Disclosure](https://portswigger.net/web-security/access-control/lab-user-id-controlled-by-request-parameter-with-password-disclosure)

**Solution:**
1. Log in and observe that the response contains password-related information.
2. Modify the request to access this sensitive information.

## Insecure direct object references
### Reference: [PortSwigger: Insecure Direct Object References](https://portswigger.net/web-security/access-control/lab-insecure-direct-object-references)

**Solution:**
1. Try accessing objects by directly manipulating the URL parameters.
2. Observe if the application allows unauthorized access to restricted objects.

## Multi-step process with no access control on one step
### Reference: [PortSwigger: Multi-Step Process with No Access Control on One Step](https://portswigger.net/web-security/access-control/lab-multi-step-process-with-no-access-control-on-one-step)

**Solution:**
1. Follow the multi-step process while monitoring requests in Burp Suite.
2. Observe that one of the steps lacks proper access control.
3. Exploit the vulnerability to bypass the process.

## Referer-based access control
### Reference: [PortSwigger: Referer-based Access Control](https://portswigger.net/web-security/access-control/lab-referer-based-access-control)

**Solution:**
1. Try accessing a restricted resource and observe the use of the `Referer` header for access control.
2. Modify the `Referer` header in your requests to bypass the access control mechanism.
3. You can use tools like **Burp Suite** to intercept and modify the HTTP request to change the `Referer` header value to that of a valid resource.
4. Once the `Referer` header is modified to a valid value, the application should allow you to access the restricted resource.
5. Retrieve the sensitive information or perform the necessary actions, like deleting the user Carlos, depending on the lab's objective.
